...
Let's move on to new material. Find the closest pairs of categories using the
angle distance. Remember to set the document variable to "label". What do you
make of the pairs?
...
results as a JSON file and go to the link below to visualise the results.
...
```{r, question-09}
dsst_umap(anno) %>% dsst_json_drep(docs)
```
