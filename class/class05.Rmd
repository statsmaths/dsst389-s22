...
a winner). Unlike linear regression, there is no specific *model* per say.
...
important than common words (with larger counts). The opposite would occur if
we used the raw counts themselves. We will revisit the details in a future
set of notes.
...
By default, the function uses a form of cross-validation to choose the best
value of k. For text classification, it is very often the case that the best
k is equal to 1. We can manually set a different k by setting the argument
`k` in the function call.

With a little bit of R code, we can plot the cross-validation curve.

```{r}
model$cv_curve %>%
  ggplot(aes(x = k)) +
    geom_line(aes(y = cv_train), linetype = "dashed") +
    geom_line(aes(y = cv_valid)) +
    labs(y = "Error Rate")
```
