...
```{r, message=FALSE}
docs <- read_csv("../data/cds.csv.gz")
anno <- read_csv("../data/cds_token.csv.gz")
```
...
this with typical Euclidean distances. However, as we mentioned with KNN, this
would by heavily biased based on the number of words in each document and would
be heavily influence by the most frequent terms.

A better approach is to look at the angle between two documents as projected in
a high-dimensional space. We can compute angle distances that with the function
`dsst_angle_dist()`:

```{r}
dsst_angle_dist(anno)
```

The default associates each document with its one closest document. We can
extend this to a larger set of document by selecting a different parameter
`n`. We will see in a moment how we might learn something from these distances.
For now, let us move onto a slightly different task.
...
file and visualise it using a tool writen in JavaScript.

To create the data to load, run the following function:

```{r}
dsst_pca(anno) %>% dsst_json_drep(docs)
```
...
produce reviews. However, we could collect the tokens across another metadata
...
Let's see some results. For example, here are the users that are the most
similar to one another:

```{r}
dsst_angle_dist(anno, doc_var = "user_name")
```

When dimensionality reduction is plotted with fewer than 100 items, it also
includes labels that help us understand the plot without using the JavaScript
interface (though the latter is still available):

```{r, warning = FALSE}
dsst_pca(anno, doc_var = "user_name") %>% dsst_plot_dred()
```
...
```{r, warning = FALSE}
dsst_umap(anno, doc_var = "user_name") %>% dsst_plot_dred()
```
